...
export const PaymentProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [gasPriceData, setGasPriceData] = useState<any>(6);
  ...
  const wagamiProvider = useProvider();
  ...
  useEffect(() => {
    if (wagamiProvider.network) {
      wagamiProvider
        .getFeeData()
        .then((res) => {
          const { maxFeePerGas } = res;
          setGasPriceData({
            maxFeePerGas,
          });
        })
  ...
  useEffect(() => {
    ...
    payRequest((request as any).raw, library as any, undefined, gasPriceData)
      .then(txCallback)
      .catch((e) => {
        setPaying(false);
        console.log(e);
  ...
  useEffect(() => {
    ...
    approveErc20((request as any).raw, library as any, gasPriceData)
      .then(async () => {
        setBroadcasting(true);
        setApproved(true);
  ...
  useEffect(() => {
    if (
      ...
      runChecks((request as any).raw, account, library, approved).then(
        (err: any) => {
          setError(err);
          setReady(true);
